...
public class EmailUtil {
    ...
    private boolean auth = Boolean.valueOf(ConfigUtil.getValue("email.send.auth"));
    private String mail_host_account =ConfigUtil.getValue("email.send.account");
    private String mail_host_password = ConfigUtil.getValue("email.send.password");;
    ...
    private EmailUtil(String mail_host){
        this.mail_host = mail_host;
    }

    private EmailUtil(String mail_host, boolean auth, String account, String password){
        this(mail_host, 25, EncryptionTypes.Default.ordinal(), auth, account, password);
    }

    private EmailUtil(String mail_host, int mail_port, int encryptionType, boolean auth, String account, String password){
        this.mail_host = mail_host;
    ...
    private EmailUtil(String mail_host, boolean auth, String account, String password, boolean isHtml){
        this(mail_host, 25, EncryptionTypes.Default.ordinal(), auth, account, password, isHtml);
    }

    private EmailUtil(String mail_host, int mail_port, int encryptionType, boolean auth, String account, String password, boolean isHtml){
        this.mail_host = mail_host;
        this.mail_port = mail_port;
        this.encryptionType = encryptionType;
        this.auth = auth;
    ...
    public void sendEmail(String senderAddress,
        ...
        try {
            ...
            if(this.getDefaultIsHtml()){
                contentPart.setContent("<html lang='zh-CN'><meta http-equiv=Content-Type content=text/html; charset="+default_charset+"><head ><meta charset='utf-8'></head><body>"+msg+"</body></html>","text/html;charset="+default_charset);
            }else{
    ...
    private Properties getProperties(){
        ...
        if(defaultEncryptionType == EncryptionTypes.TLS.ordinal()){
            ...
            props.put("mail.smtp.debug", ConfigUtil.getValue("email.smtp.debug"));
            props.put("mail.smtp.auth", String.valueOf(this.auth));
