...
public class ProcessCoreServiceImpl implements ProcessCoreService {
    private Logger logger=Logger.getLogger(ProcessCoreServiceImpl.class);
    @Autowired
    ...
    public void backCurrentProcess(String taskId,Map<String,Object> varoables){
        ...
        if(task!=null){
            ExecutionEntity entity= (ExecutionEntity) runtimeService.createExecutionQuery().executionId(task.getExecutionId()).singleResult();
            try {

                backProcess(taskId,entity.getActivityId(),varoables);
            } catch (Exception e) {
                logger.error(e);
                e.printStackTrace();
            }
     ...
     * @throws Exception
     */
    private void backProcess(String taskId, String activityId,
