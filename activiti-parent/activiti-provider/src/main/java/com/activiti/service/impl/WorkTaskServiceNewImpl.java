...
public class WorkTaskServiceNewImpl implements WorkTaskNewService {
    ...
    public PageInfo<Task> queryByAssign(String userId,int startPage,int pageSize,TaskQueryEntity taskQueryEntity) throws WorkFlowException {
        ...
        if(StringUtils.isNotBlank(userId)){
            ...
            //业务主键
            if(StringUtils.isNotBlank(taskQueryEntity.getBussinessType())){
            ...
            nativeTaskQuery = nativeTaskQuery.sql(sb.toString())
                    .parameter("userIdLike","%"+userId+"%")
                    .parameter("userId",userId)
                    .parameter("taskStatus",userId + ":" + TaskStatus.UNFINISHED.value)
                    .parameter("businessKey","%"+taskQueryEntity.getBussinessType()+"%");
            count = nativeTaskQuery.count();
            taskList = nativeTaskQuery.listPage((startPage-1)*pageSize, pageInfo.getSize());
    ...
    @Override
    public boolean checkBunessKeyIsInFlow(TaskQueryEntity taskQueryEntity, String businessKey) {
